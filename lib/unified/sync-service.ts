...
export class SyncService {
  ...
  async syncAgent(agentId: string, direction: 'push' | 'pull' | 'both' = 'both'): Promise<boolean> {
    try {
      if (direction === 'push' || direction === 'both') {
        ...
        if (localError || !localAgent) {
          console.error('Error fetching local agent:', localError)
          return false
        }
        ...
        if (unifiedAgent.externalId) {
          ...
          if (!response.success) {
            console.error('Failed to update agent in external API:', response.error)
            await this.updateSyncStatus(agentId, 'error')
            return false
          }
        } else {
          // Create new
          ...
          if (response.success && response.data) {
            ...
            console.error('Failed to create agent in external API:', response.error)
            await this.updateSyncStatus(agentId, 'error')
            return false
          }
        }
  ...
  async syncPendingAgents(): Promise<void> {
